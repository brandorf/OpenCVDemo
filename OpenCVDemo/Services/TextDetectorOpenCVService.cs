...
public class TextDetectorOpenCvService : IVideoProcessingService
{
    private int _currentFrame = 0;
    private int _lastFrame = 1;
    private TimeSpan _frameTime = TimeSpan.Zero;
    private readonly TextDetectorOpenCvServiceConfiguration _config;

    public TextDetectorOpenCvService(IOptions<TextDetectorOpenCvServiceConfiguration> config)
    {
    ...
    public async Task ProcessVideo(string videoFilePath)
    {
        ...
        // Load the pre-trained TextDetectorCNN model for text detection
        var textDetector = TextDetectorCNN.Create(prototextPath, modelCombinedPath );
        ...
        while (true)
        {
            ...
            if (previousFrame == null || !AreFramesSimilar(frame, previousFrame))
            {
                ...
                // Detect text regions
                textDetector.Detect(frame, out Rect[] boxes, out float[] confidences);

                List<Rect> filteredBoxes = new List<Rect>();
                for (int i = 0; i < boxes.Length; i++)
                {
                    if (confidences[i] >= _config.ConfidenceThreshold)
                    {
